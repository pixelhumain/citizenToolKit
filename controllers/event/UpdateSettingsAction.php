...
class UpdateSettingsAction extends CAction
{
    public function run()
    {
        ...
        if(isset($_POST["type"])){
          $res=Preference::updateConfidentiality(Yii::app()->session["userId"],Event::COLLECTION,$_POST);
        }else{
        	$res=Preference::updatePreferences(Yii::app()->session["userId"],Event::COLLECTION);
        }
		Rest::json($res);
		exit;
    }
